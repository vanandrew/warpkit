...
def main():
    ...
    parser.add_argument(
        "--metadata", nargs="+", required=True, help="JSON sidecar for each echo"
    )
    parser.add_argument(
        "--out_prefix", help="Prefix to output field maps and displacment maps."
    )
    parser.add_argument(
        "-f", "--noiseframes", type=int, default=0, help="Number of noise frames"
    )
    parser.add_argument(
        "-n", "--n_cpus", type=int, default=4, help="Number of CPUs to use."
    )
