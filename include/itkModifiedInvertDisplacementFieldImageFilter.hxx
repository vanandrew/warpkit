 ...
 *=========================================================================*/
/*
  This is a modified version of itkInvertDisplacementFieldImageFilter.hxx
*/
#ifndef itkModifiedInvertDisplacementFieldImageFilter_hxx
#define itkModifiedInvertDisplacementFieldImageFilter_hxx
...
/*
 * ModifiedInvertDisplacementFieldImageFilter class definitions
 */
template <typename TInputImage, typename TOutputImage>
ModifiedInvertDisplacementFieldImageFilter<TInputImage, TOutputImage>::ModifiedInvertDisplacementFieldImageFilter()
    : m_Verbose(false),
...
template <typename TInputImage, typename TOutputImage>
void ModifiedInvertDisplacementFieldImageFilter<TInputImage, TOutputImage>::SetInterpolator(
...
template <typename TInputImage, typename TOutputImage>
void ModifiedInvertDisplacementFieldImageFilter<TInputImage, TOutputImage>::GenerateData() {
    ...
    if (this->m_Verbose) {
        std::cout << "Maximum number of iterations: " << this->m_MaximumNumberOfIterations << std::endl;
        std::cout << "Max error tolerance threshold: " << this->m_MaxErrorToleranceThreshold << std::endl;
        std::cout << "Mean error tolerance threshold: " << this->m_MeanErrorToleranceThreshold << std::endl;
    }
    ...
    while (iteration++<this->m_MaximumNumberOfIterations&& this->m_MaxErrorNorm> this->m_MaxErrorToleranceThreshold &&
        ...
        if (this->m_Verbose) {
            std::cout << "Iteration " << iteration-1 << ": mean error norm = " << this->m_MeanErrorNorm
                      << ", max error norm = " << this->m_MaxErrorNorm << std::endl;
        }
...
template <typename TInputImage, typename TOutputImage>
void ModifiedInvertDisplacementFieldImageFilter<TInputImage, TOutputImage>::DynamicThreadedGenerateData(
            ...
            if (this->m_EnforceBoundaryCondition) {
                for (unsigned int d = 0; d < ImageDimension; ++d) {
                    if (index[d] == startIndex[d] ||
                        index[d] == static_cast<IndexValueType>(size[d]) - startIndex[d] - 1) {
                        ItI.Set(zeroVector);
                        break;
                    }
                }
...
template <typename TInputImage, typename TOutputImage>
void ModifiedInvertDisplacementFieldImageFilter<TInputImage, TOutputImage>::PrintSelf(std::ostream& os,
                                                                                      Indent indent) const {
